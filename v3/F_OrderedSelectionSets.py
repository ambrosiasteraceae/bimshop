...
for item in container:
    model_items = container[item]
    if item in slabs or item in piles:
        sorted_mitems = process_cluster(model_items)

    for j, child in enumerate(model_items):
        col = ModelItemCollection()
        col.Add(child)
        newsel = SelectionSet(col)

        if item in slabs:
            name_ref = rename_map[item] + '-' + str(sorted_mitems[j+1])

        elif item in piles:
            name_ref = "(" + str(sorted_mitems[j+1]) + ")" + rename_map[item] + '-' +\
                       str(indexed[sorted_mitems[j+1]])

        elif item in chainages:
            name_ref = item.split()[-1] + ' ' + chainages[item][j + 1]

        else:
            name_convention = item + '-' + str(j + 1)
